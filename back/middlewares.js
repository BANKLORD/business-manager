...
const authenticated = (req, res, next) => {
  ...
  if (!token) return res.status(403).json({error: "Veuillez vous reconnecter"})
  else {
...
const hasPermission = (permission) => {
  return async ( req, res, next ) => {
    ...
    if ( role && permissions?.find(perm => perm.name == 'dev') ) {
      next();
    }
    else if ( role && permissions?.find(perm => perm.name == permission) ) {
      next();
    } else {
      return res.status(419).json("Pas d'accès")
    }
  }
}
