...
export default {
  ...
  computed: {
    ...
    showCreateOrUpdateCardText() {
      return (this.showCreateOrUpdateCard < 0) ? `Création d'un nouveau Fournisseur`:`Modification de: ${this.selectedItem?.companyName ?? this.selectedItem?.name}`;
    },
  },
  data: () => {
    return {
      ...
      searchTerm: '',
      selectedType: -1,
      listTitle: 'List des fournisseurs',
      addNewButtonText: 'Nouveau Fournisseur',
      selectedItem: {
        id: 0
      },
      columns: [
  ...
  methods: {
    ...
    createOrUpdate() {
      ...
      this.selectedItem.type = 'Provider';
