    ...
    <v-row class="text-center" justify="center" max-width="1024px">
      ...
      <v-col cols="12" md="8">
        <v-card>
          ...
          <div class="text-center my-4">
            <v-btn color="primary" @click="generatePDF"> Générer un PDF <v-icon right>fa-print</v-icon></v-btn>
          </div>
          <v-divider></v-divider>
          <v-data-table
            :headers="columns"
          ...
          >
            ...
            <template v-slot:[`item.in`]="{ item }">
              {{ item.in | formatQuantity }}
            </template>
            <template v-slot:[`item.out`]="{ item }">
              {{ (item.out - item.refunded) | formatQuantity }}
            </template>
            <template v-slot:[`item.rest`]="{ item }">
              {{ (item.in + item.refunded) - item.out | formatQuantity }}
            </template>
            <template v-slot:[`item.investments`]="{ item }">
              {{ item.investments  | formatPrice }}
            </template>
      ...
      <v-col cols="12" md="4">
        <v-card>
          ...
          <PnlTable
            :totalInvestments="items.reduce((acc, item) => acc + item.investments, 0)"
            :totalProfit="items.reduce((acc, item) => acc + item.profit, 0)"
            :restStockWorth="items.reduce((acc, item) => acc + (item.retail_price * ((item.in + item.refunded) - item.out)), 0)"
          ></PnlTable>
                ...
                <ul>
                  ...
                  >
                    <li
                      ...
                      :key="'op_' + op.id"
                    >
                      <span> {{op.inventory.id}} </span> (-{{ op.quantity }})
                    </li>
                  </template>
                </ul>
              </template>
...
import PnlPieChart from '@/components/stock/pnl-pie-chart.vue';
import PnlTable from '@/components/stock/pnl-table.vue';

export default {
  components: { PnlPieChart, PnlTable },
  watch: {
    searchTerm() {
  ...
  methods: {
    generatePDF() {
      ...
      const table = this.$refs.statusTable.$el.querySelector('table')
      doc.autoTable({
        html: table,
        startY: 60
      })
      doc.save(`stock-status-${Date.now()}`)
    },
    searchTermChanged() {
      this.loadProducts();
    },
    loadProducts() {
      ...
      /** */
      this.axios.get(`${process.env.VUE_APP_API_URL}/movements`, { params: searchParams }).then(res => {
        this.items = res.data;
      });
    },
    loadItems() {
