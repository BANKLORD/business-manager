...
export default {
  ...
  computed: {
    ...
    showCreateOrUpdateCardText() {
      return this.showCreateOrUpdateCard ? `Création d'un nouveau Client / Fournisseur`:`Modification de: ${this.selectedItem?.companyName ?? this.selectedItem?.name}`;
    },
  },
  data: () => {
    return {
      ...
      searchTerm: '',
      selectedType: -1,
      listTitle: 'Client / Fournisseur',
      addNewButtonText: 'Nouveau Client / Fournisseur',
      selectedItem: {
        id: 0
      },
      columns: [
  ...
  methods: {
    ...
    loadItems() {
      this.axios.get(`${process.env.VUE_APP_API_URL}/concerns`).then(res => {
        this.items = res.data;
      });
    },
    showCreateCard() {
