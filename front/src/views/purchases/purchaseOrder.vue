          ...
          >
            <v-menu offset-y>
                ...
                >
                  ...
                  <v-list-item class="d-block" link @click="showPurchaseOrder(item)">
                    <v-list-item-title>
                      <v-btn
                        text
                        color="success"
...
export default {
  ...
  data: () => {
    return {
      ...
      dropzoneOptions: {
        ...
        /** Changeable */
        url: `${process.env.VUE_APP_API_URL}/purchase/purchaseOrder`,
        method: 'PUT',
        thumbnailWidth: 150,
      },
      products: [],
      ...
      providers: [],
      selectedProvider: -1,
      listTitle: 'Management de bons de commande',
      addNewButtonText: 'Nouveau bon de commande',
      searchTerm: '',
      selectedType: -1,
      selectedItem: {
        id: 0,
  ...
  methods: {
    ...
    showItemInformation(item) {
      this.loadingDialog = true;
      this.axios.get(`${process.env.VUE_APP_API_URL}/purchase/purchaseOrder/${item.id}`).then( res => {
    ...
    reloadSelectedItem() {
      this.axios.get(`${process.env.VUE_APP_API_URL}/purchase/purchaseOrder/${this.selectedItem.id}`).then(res => {
    ...
    loadPurchases() {
      ...
      /** */
      this.axios.get(`${process.env.VUE_APP_API_URL}/purchase/purchaseOrders`, { params: searchParams }).then(res => {
        this.items = res.data;
      });
    },
    loadItems() {
      this.axios.get(`${process.env.VUE_APP_API_URL}/purchase/purchaseOrders`).then(res => {
        this.items = res.data;
      });
    ...
    createOrUpdate() {
      ...
      const url =
        (id < 0) ?
        `${process.env.VUE_APP_API_URL}/purchase/purchaseOrder`:(id > 0) ?
        `${process.env.VUE_APP_API_URL}/purchase/purchaseOrder/${id}`:null
    ...
    showDeleteConfirm(item) {
      this.$swal({
        ...
        if (result.isConfirmed) {
          this.axios.delete(`${process.env.VUE_APP_API_URL}/purchase/purchaseOrder/${item.id}`).then(() => {
            this.$swal(
              'Supprimé!',
