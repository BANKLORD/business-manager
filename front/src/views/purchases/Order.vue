    ...
    <v-row class="text-center" justify="center" max-width="1444px">
      <v-card>
        ...
        <v-row class="mx-3 my-2" justify="center">
          ...
          <v-col>
            <DatePicker label="Date de début" :bound="1" v-model="selectedEndDate"></DatePicker>
          </v-col>
        </v-row>
        <v-row class="mx-3 my-2" justify="center">
          <v-col>
          ...
          >
            <v-menu offset-y>
                ...
                >
                  ...
                  <v-list-item class="d-block" link @click="transferToPurchase(item)">
                    <v-list-item-title>
                      <v-btn
                        text
                        color="primary"
                        ...
                        :disabled="loading"
                      >
                        <v-icon left>fas fa-cart-shopping</v-icon> Crée une achat
                      </v-btn>
                    </v-list-item-title>
                  </v-list-item>
                  <!-- Edit Button -->
            ...
            <tbody>
              ...
              >
                <td class="text-left">
                  <ul>
                    <li
                      v-for="pCode in products.find(p => p.id == purchaseProduct.productId).productCodes"
                      :key="'pcode' + pCode.id"
                      v-text="pCode.code"
                    >
                    </li>
...
export default {
  ...
  data: () => {
    return {
      ...
      dropzoneOptions: {
        ...
        /** Changeable */
        url: `${process.env.VUE_APP_API_URL}/purchase/order`,
        method: 'PUT',
        thumbnailWidth: 150,
      },
      products: [],
      ...
      providers: [],
      selectedProvider: -1,
      listTitle: 'List des commandes',
      addNewButtonText: 'Nouvelle commande',
      searchTerm: '',
      selectedType: -1,
      selectedItem: {
        id: 0,
  ...
  methods: {
    ...
    showProducts(purchase) {
      this.productsListDialog = true;
      this.selectedPurchase = purchase;
    },
    async transferToPurchase(purchase) {
      const result = await this.$swal({
        title: 'Veuillez confirmer',
        text: `En cliquant sur confirmer, vous confirmez la création d'une achat pour la commande #${purchase.id}`,
        icon: 'info',
      ...
      if (result?.isConfirmed) {
        ...
        this.axios({
          url: `${process.env.VUE_APP_API_URL}/purchase/order/${purchase.id}/purchase`,
          method: 'POST',
        }).then(() => {
          this.$swal(
            'L\'achat a été créé !',
            'L\'achat a été créé avec succès, vous pouvez le voir dans la section D\'achats ',
            'success'
          );
    ...
    async transferToPurchaseOrder(purchase) {
      const result = await this.$swal({
        title: 'Veuillez confirmer',
        text: `En cliquant sur confirmer, vous confirmez la création d'un bon de commande pour la commande #${purchase.id}`,
        icon: 'info',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
      ...
      if (result?.isConfirmed) {
        this.loadingDialog = true;
        this.axios({
          url: `${process.env.VUE_APP_API_URL}/purchase/order/${purchase.id}/purchaseOrder`,
          method: 'POST',
        }).then(() => {
          this.$swal(
    ...
    showItemInformation(item) {
      this.loadingDialog = true;
      this.axios.get(`${process.env.VUE_APP_API_URL}/purchase/order/${item.id}`).then( res => {
    ...
    reloadSelectedItem() {
      this.axios.get(`${process.env.VUE_APP_API_URL}/purchase/order/${this.selectedItem.id}`).then(res => {
    ...
    loadPurchases() {
      ...
      /** */
      this.axios.get(`${process.env.VUE_APP_API_URL}/purchase/orders`, { params: searchParams }).then(res => {
        this.items = res.data;
      });
    },
    loadItems() {
      this.axios.get(`${process.env.VUE_APP_API_URL}/purchase/orders`).then(res => {
        this.items = res.data;
      });
    ...
    createOrUpdate() {
      ...
      const url =
        (id < 0) ?
        `${process.env.VUE_APP_API_URL}/purchase/order`:(id > 0) ?
        `${process.env.VUE_APP_API_URL}/purchase/order/${id}`:null
    ...
    showDeleteConfirm(item) {
      this.$swal({
        ...
        if (result.isConfirmed) {
          this.axios.delete(`${process.env.VUE_APP_API_URL}/purchase/order/${item.id}`).then(() => {
            this.$swal(
              'Supprimé!',
