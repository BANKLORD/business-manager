...
export default {
  ...
  methods: {
    ...
    generateInvoiceInformation() {
      ...
      this.invoiceData.content.push({
        text: [
          "Avoir N°: ",
          { text: `${new Date().getFullYear() + '.' + this.sale.count}`, style: [ 'bold' ] },
          " / Facture N°: ",
          { text: `${new Date().getFullYear() + '.' + this.invoice.count}\n`, style: [ 'bold' ] },
          "Date: ",
  ...
  created() {
    this.sale = this.invoice.refunds[0];
    this.company = this.$store.getters.getCompany;
    this.sale.saleProducts = this.sale.refundOperations.map(refundOperation => ({
        quantity: refundOperation.quantity,
        price: refundOperation.saleProduct.price,
        product: refundOperation.saleProduct.product
      })
    );
    for (const saleProduct of this.sale.saleProducts) {
