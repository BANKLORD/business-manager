<template>
  <div>
    ...
    >
      ...
      <v-toolbar-title>Bon de livraison N° {{sale.count}}</v-toolbar-title>
      <v-spacer></v-spacer>
      <v-toolbar-items>
        <v-menu offset-y>
...
export default {
  ...
  methods: {
    ...
    async downloadQuotation() {
      await pdfMake.createPdf(this.invoiceData).download(`BL-2024.${this.sale.count}.pdf`);
    },
    async generatePDF(){
    ...
    generatePriceToLetters() {
      ...
      this.invoiceData.content.push([
        { text: 'Arrêtée la présente bon de livraison à la somme de: \n', bold: true, fontSize: '10' },
    ...
    async generateQuotation() {
      ...
      this.invoiceData = Object.assign({}, this.defaultInvoiceData);
      this.invoiceData.info.title = `BL-2024.${this.sale.count}`;
