<template>
  <div>
    ...
    >
      ...
      <v-toolbar-title>Facture N° {{sale.count}}</v-toolbar-title>
      <v-spacer></v-spacer>
      <v-toolbar-items>
        <v-menu offset-y>
            ...
            <v-list-item class="d-block">
              <v-list-item-title>
                <v-switch
                  ...
                  label="Basculer l'en-tête/Pied de page de la facture"
                >
                </v-switch>
              </v-list-item-title>
            </v-list-item>
...
export default {
  ...
  computed: {
    productsHasDiscount() {
      ...
      // for (const deliveryForm of this.sale.deliveryForms) {
  ...
  methods: {
    ...
    async downloadQuotation() {
      await pdfMake.createPdf(this.invoiceData).download(`FACTURE-2024.${this.sale.count}.pdf`);
    },
    async generatePDF(){
    ...
    generateInvoiceInformation() {
      ...
      this.invoiceData.content.push({
        text: [
          ...
          "Date: ",
          { text: `${Vue.filter('formatDate')(new Date(this.sale.createdAt).toISOString().split('T')[0])}`, style: [ 'bold' ] },
        ],
    ...
    async generateQuotation() {
      ...
      this.invoiceData = Object.assign({}, this.defaultInvoiceData);
      this.invoiceData.info.title = `FACTURE-2024.${this.sale.count}`;
  ...
  created() {
    ...
    for (const deliveryForm of this.sale.deliveryForms) {
      const sale = deliveryForm.sale;
      this.vatPercent = deliveryForm.sale.vat;
      for (const saleProduct of sale.saleProducts) {
