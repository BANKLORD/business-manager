<template>
  <div>
    <div class="text-center my-3">
      <h3> Bon de livraison N° {{ deliveryFormInformation.count }}</h3>
      <v-row justify="center" class="my-3">
        <v-col md="2">
          <v-btn outlined color="primary" style="margin: auto;" @click="download">
    ...
    >
      ...
      <table style="width: 100%">
        ...
        <td class="invoice-information">
          <p><b>{{ invoiceName }} N°:</b> {{ deliveryFormInformation.count }}</p>
          <p><b>Date:</b> {{ deliveryFormInformation.createdAt | formatDate }}</p>
        </td>
      </table>
      <div class="products">
        <v-simple-table dense>
          ...
          <tbody>
            <tr v-for="saleProduct of saleProducts" :key="'saleProduct_' + saleProduct.id">
              ...
              <td>{{ saleProduct.product[saleProduct.type] | formatPrice }}</td>
...
export default {
  computed: {
    ...
    productsHasDiscount() {
      ...
      for (const saleProduct of this.saleProducts) {
        if ( saleProduct.price < saleProduct.product[saleProduct.type] )
          hasDiscount = true;
      }
      return hasDiscount;
    },
  ...
  props: {
    ...
    deliveryFormInformation: Object,
  },
  data() {
    return {
      company: {},
      sale: this.deliveryFormInformation.sale,
      saleProducts: this.deliveryFormInformation.sale.saleProducts,
    }
  },
  created() {
  ...
  methods: {
    download() {
      html2Canvas(this.$refs.deliveryForm, {
        ...
        doc.save(`bl-${this.deliveryFormInformation.count}.pdf`);
      })
    }
  }
}
